...
public class ConvertLocation {
    ...
    public static GridLocation convertToGrid(double lat, double lon) {
        ...
        double rs = Math.tan(Math.PI * 0.25 + lat * DEGRAD * 0.5);
        ...
        int nx = (int) Math.floor(rs * Math.sin(theta) + XO + 0.5);
        int ny = (int) Math.floor(ro - rs * Math.cos(theta) + YO + 0.5);

        return new GridLocation(nx, ny);
    }
    ...
    public static class GridLocation {
        ...
        public GridLocation(int nx, int ny) {
            this.nx = nx;
            this.ny = ny;
        }
    }
